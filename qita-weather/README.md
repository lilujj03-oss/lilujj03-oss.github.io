...
- 介面採柔和淺粉紅背景、深藍文字與藍綠功能色，維持清楚易讀的資訊層級。
- 即時資訊卡顯示溫度、濕度、風向風速、氣壓、天氣現象與最近 1 小時雨量。
- 提供「縣市 → 行政區」連動雙層選單，篩選後自動縮放地圖並切換至區內測站。
- 分開顯示「中央氣象署最新觀測時間」與「瀏覽器本次同步時間」。
- 比較每筆時間戳，避免較舊 API 資料覆蓋較新的官方快照。

## 完整全端資料流程

| 階段 | 核心元件 | 說明 |
| --- | --- | --- |
| SOURCE | 中央氣象署 API | 三組官方開放資料 |
| PIPELINE | Python Parser | 清理、整點分桶、配對 |
| STORAGE | SQLite | 歷史保存與索引 |
| SERVICE | FastAPI | REST API 與排程 |
| VIEW | Leaflet + Plotly | 地圖與互動圖表 |

## 三組官方資料

- `O-A0001-001`：氣象測站逐時觀測。
- `O-A0002-001`：過去 1 小時雨量。
- `F-D0047-091`：各縣市一週預報。
...
GitHub 每小時排程只從 Repository Secret `CWA_API_KEY` 讀取授權碼，不會將金鑰寫入 HTML、JavaScript、JSON 或 Git commit。AI 趨勢功能僅供技術展示，不是中央氣象署正式預報。
...
[閱讀完整重點紀錄](./CONVERSATION-HIGHLIGHTS.md)
